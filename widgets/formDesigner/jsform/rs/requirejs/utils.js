 ...
 * json 方法拓展模块化  结束
 */
define("jquery_extends",["jquery"],function(V){
...
define("jquery_extends2",["jquery"],function(b){
	 ...
	 b.postJSON = function(d, f, g, e) {
	    ...
	    //悬浮提示框
	    b.showPop = function(text,timeOut){
	    	var $div = $("#js-modify-success");
	    	if($div.length==0){
	    	  $div = $('<div id="js-modify-success" class="pop-con-save center" style="z-index: 1001; display: none;"><div>'+text+'</div><div class="but-div"><a href="javascript:void(0);" clss="tipBut"></a></div></div>');
	    	}else{
	    		$div.html(text);
	    		$div.show();
	    	}
	    	//定时任务
	    	setTimeout('$.hidePop()',2*1000);
	    };
	    //隐藏pop提示框
	    b.hidePop = function(){
	    	var $div = $("#js-modify-success");
	    ...
	    b.lightBox = function(k) {
	        var e = {url: "",overlay: "#overlay",content: "#lbContent",lightBox: "#lightBox",btnConfirm: "#btnConfirm",btnCancel: "#btnCancel",choiceMenu:undefined,choiceArray:undefined,
	        	//自定义方法：创建类别
        		createChoiceMenu:function(choiceMenu,choiceArray){
        			if(choiceArray && choiceMenu){
        				b(choiceMenu).empty();
            			b.each(choiceArray,function(index,choice){
            				var $a = b("<a>").text(choice.text).data("list",choice.list);
            				b("<li>").append($a).appendTo(b(choiceMenu));
            			});
        	        	//点击事件绑定
        	        	b(d.choiceMenu).find("a").unbind("click").click(function(event){
        	            	var html = "";
        	        		var list = b(this).data("list").split(";");
        	        		b.each(list,function(i,v){
        	            		 html+=(v+"\n");
        	        		});
	        ...
	        if (d.url) {
	            if (b.data(document, d.url)) {
	                ...
	                setTimeout(function() {
	                    d.loaded();
	                    d.createChoiceMenu(d.choiceMenu,d.choiceArray);
	                }, 20)
	            } else {
	                if (d.url.indexOf("#") == 0) {
...
define(["date_utils", "json_utils","jquery_extends","jquery_extends2"], function () {
	...
	return {
		...
		//真实长度，一个汉字两个字节，一个字母和数字一个字节
		subRealLen2:function(pStr, pLen){
		 ...
		 * totalLen:汉字标准长度
		 */
		subRealLen:function(str,totalLen){
		    var len = 0;
		    debugger;
		    for (var i=0; i<str.length; i++) {
			     var c = str.charCodeAt(i); 
