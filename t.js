...
(function () {
    $.formatPrice = function (g, f, e, d, a, b) {
        ...
        if ("1" == d) {
            ...
            g.find("a.decrease,a.increase").css({display: "none"});
            g.find("span.number-pre").show();
            g.find("input.number").addClass("noincrease")
        } else {
            f = f || "0";
            g.find("div.price").text(e + f + (a ? "/" + a : ""));
            g.find("a.decrease,a.increase").css({display: "inline-block"});
...
function resizeHandle(b) {
    var a = function (c) {
        ...
        f.position({of: c, my: "left top", at: "left top"});
...
function propertyInit() {
    ...
    $("#N").change(function () {
        ...
        $("#f" + IDX).find("div.content>select").css({width: (100 / X - 1) + "%", "margin-left": "1%"});
        CHANGED = true
    });
        ...
        click: function () {
            ...
            if (V.hasClass("hide")) {
                ...
                if (U.find("div.item-upload").length > 0) {
                    U.find("a.icononly-add,a.icononly-del,a.icononly-mov,a.icononly-limit").css({"margin-top": "-32px"})
                }
            } else {
                V.addClass("hide");
                if (U.find("div.item-upload").length > 0) {
                    U.find("a.icononly-add,a.icononly-del,a.icononly-mov,a.icononly-limit").css({"margin-top": "-14px"})
                }
            }
        }
    });
    ...
    $("#likertRows>li>:text").live({keyup: s, change: s});
    var b = function (V) {
            ...
            if (F[IDX].ITMS[V].NM && M.MXID > 0) {
                $.confirm({
                    msg: delConfirmMsg, showyes: true, confirm: function () {
                        $.showStatus("正在执行删除操作 ...");
                        var Y = "deleteItem";
                        $.postJSON(Y, {_id: M._id, FLDID: F[IDX].FLDID, NM: F[IDX].ITMS[V].NM}, function () {
                            b(W);
                            $.hideStatus()
                        })
                    }
    ...
    $("#likertCols>li>:text").live({keyup: B, change: B});
            ...
            if (F[IDX].ITMS[0].ITMS[U].ITMID && M.MXID > 0) {
                $.confirm({
                    msg: delConfirmMsg, showyes: true, confirm: function () {
                        ...
                        $.postJSON(X, {_id: M._id, FLDID: F[IDX].FLDID, COLINDEX: U}, function () {
                            O(V);
                            $.hideStatus()
                        })
                    }
        ...
        if (F[IDX].ITMS[W].NM || F[IDX].ITMS[W].ITMID && M.MXID > 0) {
            $.confirm({
                msg: delConfirmMsg, showyes: true, confirm: function () {
                    ...
                    if (F[IDX].ITMS[W].NM) {
                        ab = {_id: M._id, FLDID: F[IDX].FLDID, NM: F[IDX].ITMS[W].NM};
                        if (F[IDX].ITMS[W].IMG) {
                            ab.IMG = F[IDX].ITMS[W].IMG
                        }
                    } else {
                        ab = {_id: M._id, FLDID: F[IDX].FLDID, ITMID: F[IDX].ITMS[W].ITMID, TYP: F[IDX].TYP}
                    }
    ...
    $("a.icononly-del", "#itemList2").live({click: f});
    $("a.icononly-del", "#itemList").live({click: D});
    $("a.icononly-del", "#goodsList").live({click: D});
    $("#itemList").find("a.icononly-add").live({
        click: function () {
            ...
            X.find("img.img").addClass("hide").attr({src: ""});
            X.find("span").removeClass("hide");
            $(this).parent().after(X);
            var W = $("#f" + IDX).find("div.content").children(":eq(" + V + ")");
            W.after($(W).clone().find(":radio,:checkbox").prop("checked", false).end().find("img.img").attr({src: ""}).end());
    ...
    $(":text.sl, :text.xs", "#itemList").live({keyup: I, change: I});
    $(":radio", "#itemList").live({
        click: function () {
    ...
    $("#goodsList").find("input.val").live({keyup: E, change: E});
    $("#goodsList").find("textarea.des").live({keyup: K, change: K});
    $("#goodsList").find("input.price").live({keyup: H, change: H});
    $("#goodsList").find("input.unt").live({keyup: n, change: n});
    $("#goodsList").find("input.hd").live({click: C});
    $("#goodsList").find("select.cny").live({change: g});
    $("#goodsList").find("input.def").live({keyup: Q});
    $("#goodsForBuy").click(l);
    ...
    var x = function () {
        ...
        $.ajaxFileUpload({
            ...
            fileElementId: U,
            dataType: "json",
            data: {FRMID: M._id, OLDIMG: V ? F[IDX].ITMS[W].IMG : ""},
            success: function (ac, Z) {
                if (ac.status != "success") {
                    ...
                    if (V) {
                        ...
                        F[IDX].ITMS.push({IMG: ac.filePath, VAL: "商品名称", DES: "", PRC: 0, UNT: ""})
                    }
                }
                $.hideStatus()
            },
    ...
    $("#item-upload").live({click: x});
    $("#fileToUpload").live({change: x});
    $("#goodsList input.edit-img-input").live({change: x});
    ...
    $("#uploadImage").live({
        change: function () {
            ...
            $.ajaxFileUpload({
                ...
                secureuri: false,
                dataType: "json",
                data: {OLDIMG: F[IDX].IMG, FRMID: M._id},
    ...
    $("#defval_text").bind({keyup: h, fucusout: h});
    $("#defval_number").bind({keyup: h});
    var j = function () {
        var U = $.trim($(this).val());
        if (U) {
            F[IDX].DEF = U
    ...
    $("#defval_date").bind({keyup: j});
    $("#defval_time").bind({keyup: j});
    ...
    $("#defval_phone_mobile").bind({keyup: h, change: h});
    var G = function () {
    ...
    $("#pdefval_phone_tel :input").bind({keyup: G});
        ...
        click: function () {
                        ...
                        if (X.TYP === "file") {
                            ...
                            if (X.TYP === "dropdown2") {
                                ...
                                if (X.TYP === "image") {
                                    var Z = [X.IMG];
                                    $.showStatus("正在复制字段...");
                                    $.postJSON("duplicateimages", {FRMID: M._id, IMGS: Z}, function (ac) {
                                        X.IMG = ac[0];
                                        $.hideStatus()
                                    })
                                }
            ...
            if (X.ITMS) {
                if (X.TYP === "goods") {
                    ...
                    if (Z.length > 0) {
                        $.showStatus("正在复制字段...");
                        $.postJSON("duplicategoods", {FRMID: M._id, IMGS: Z}, function (ac) {
            ...
            if (F[IDX].FLDID !== undefined && F[IDX].TYP !== "section" && F[IDX].TYP !== "html" && M.MXID > 0) {
                $.confirm({
                    msg: delConfirmMsg, showyes: true, confirm: function () {
                        $.showStatus("正在执行删除操作 ...");
                        var U = "deleteField";
                        $.postJSON(U, {_id: M._id, FLDID: F[IDX].FLDID, TYP: F[IDX].TYP}, function () {
                            R();
                            $.hideStatus()
                        })
                    }
...
function saveForm(d, e) {
    ...
    $.postJSON(b, c, function (f) {
        ...
        if (d) {
            $.lightBox({
                url: "/rs/html/formsaved.html", size: "s", loaded: function () {
                    var g = $("#divActions");
...
function addFieldsInit() {
    ...
    $("#addFromButton").click(function () {
        $("#left").effect("shake", {distance: 10, times: 3}, 20)
    });
...
function formInit() {
    ...
    $("#formName").bind({keyup: b, change: b});
    var d = function () {
        var e = $(this).val();
    ...
    $("#confirmMsg_url").bind({keyup: a, change: a});
    $("#confirmMsg_text").bind({keyup: a, change: a})
}
...
head.ready(function () {
    ...
    $("#divActions").find("a.btn").add("#btnAdvanceSearch").poptip({position: "top"});
    initWangEditor();
    createForm();
    createFields();
    fieldInit();
    ...
    $("input.number,input.money,input.price").live({keyup: a, change: a});
    setInterval(function () {
        saveForm(false)
    }, 5 * 60 * 1000);
    $("#allPropsContainer").css({maxHeight: $(window).height() - 160}).mCustomScrollbar();
    $("a.video").live({
        click: function () {
            var c = $(this).attr("videosrc");
            $.lightBox({url: "../../web/video.jsp?URL=" + c, size: "cus1"})
        }
    })
});